...
data class CreateCollectionUseCase @Inject constructor(
    val repository: FavoriteRepository
){
    suspend operator fun invoke(data:AddCollectionDTO) : Flow<CreateResponse> = flow{
        emit(repository.createCollection(data))
    }
}
